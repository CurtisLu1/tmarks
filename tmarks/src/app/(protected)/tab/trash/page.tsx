...
import { Archive, ArrowLeft, Calendar, Layers, RotateCcw, Trash2 } from 'lucide-react';
...
export default function TrashPage() {
  ...
  return (
    <div className={`h-screen flex flex-col bg-background ${isMobile ? 'overflow-hidden' : ''}`}>
      {isMobile && <MobileHeader title="回收站" showMenu={false} showSearch={false} showMore={false} />}
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            删除于{' '}
                            {group.deleted_at
                              ? formatDistanceToNow(new Date(group.deleted_at), { addSuffix: true, locale: zhCN })
                              : '未知'}
                          </span>
                        </div>
                      </div>
