...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { resolvedTheme, theme: preferredTheme } = useTheme();
  const router = useRouter();
  const pathname = usePathname();
  const { user } = useAuthStore();

  const isOnTabGroupsPage = pathname?.startsWith('/tab');
  ...
  return (
    <div
      ...
      data-theme={resolvedTheme ?? preferredTheme ?? 'light'}
    ...
    >
      ...
      <header className="h-16 sm:h-20 sticky top-0 z-50 backdrop-filter backdrop-blur-xl bg-card/80 border-b border-border/50 shadow-float flex items-center">
        <div className="w-full mx-auto px-3 sm:px-6 flex items-center justify-between">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 sm:gap-3 hover:opacity-80 transition-opacity duration-200 focus:outline-none" title="回到首页">
